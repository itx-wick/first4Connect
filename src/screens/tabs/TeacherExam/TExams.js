  ...
  const filterExamItems = (text, from, to) => {
    if (text || from || to) {
      const newData = data.filter(item => {
        ...
        console.log('Hit', text, from, to);
        if (text !== null && from !== '' && to !== '') {
          console.log('Hit1', text, from, to);
          return (
          ...
          console.log('Hit2', text, from, to);
          return (
            timeStamp >= moment(from).format('yyyy-MM-DD') &&
            timeStamp <= moment(to).format('yyyy-MM-DD')
          );
        } else if (text !== null && from === '' && to === '') {
          console.log('Hit3', text, from, to);
      ...
      .then(res => {
        if (res.Code === 2) {
          ...
          if (Object.keys(res.Data).length == 0) {
            ...
            console.log('Exams', JSON.stringify(res.Data.Exams, null, 2));
